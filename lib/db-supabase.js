...
export async function createWorkshop({ name, selected_version_ids, openrouter_model, ai_provider, system_prompt }) {
  ...
  const { data, error } = await supabase
    ...
    .insert([{
      ...
      survey_config: arguments[0].survey_config || [],
      chain_id: arguments[0].chain_id || null,
      chain_order: arguments[0].chain_order || 1,
      is_active: true,
    }])
    .select()
    .single();
