...
export function createWorkshop({ name, selected_version_ids, openrouter_model, ai_provider, maintain_version }) {
  ensureInit();
  let share_code;
  do {
...
export function createSession({ workshop_id, version_id, participant_name }) {
  ensureInit();
  const session = {
    id: randomUUID(),
    workshop_id,
    version_id,
    participant_name: arguments[0].participant_name || 'Anonymous',
    chain_user_id: arguments[0].chain_user_id || null,
    status: 'active',
    interaction_count: 0,
